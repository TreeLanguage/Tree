...
struct TestBase {
    ...
    explicit TestBase(const char* n) noexcept
        : name(n) {
...
inline void report_failure(const char* file, int line, const std::string& msg) {
    failures()++;
    std::cerr << "[FAIL] " << current_test_name() << " (" << file << ':' << line << "): " << msg
              << '\n';
}
...
inline int run_all() {
    ...
    if (failures() == 0) {
        ...
        std::cerr << failures() << " check(s) failed across " << TestBase::registry().size()
                  << " test(s).\n";
    }
    return failures();
}
...
#define CHECK_EQ(actual, expected)                                                           \
    [&] {                                                                                    \
        ...
        if (!(actual_val == expected_val)) {                                                 \
            ...
            oss << #actual << " == " << #expected << " (got " << actual_val << ", expected " \
                << expected_val << ')';                                                      \
