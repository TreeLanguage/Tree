...
const tree::Pattern& get_pattern(const tree::Program& prog, tree::PatternId id) {
    return prog.arena.get(id);
}
...
const T* require_alt(const Variant& v, const char* what) {
    ...
    if (p == nullptr) {
        tf::report_failure(__FILE__, __LINE__, std::string("expected alternative ") + what);
    }
    return p;
}
}  // namespace
...
TEST(multiplicative_binds_tighter_than_additive) {
    ...
    const auto* lhs_lit = require_alt<tree::FloatLiteral>(lhs.value, "FloatLiteral");
    if (lhs_lit != nullptr)
...
TEST(all_multiplicative_ops) {
    {
        ...
        const auto* b =
            require_alt<tree::BinaryExpr>(get_expr(r.prog, r.prog.exprs[0]).value, "BinaryExpr");
        if (b != nullptr)
...
TEST(chained_calls_are_nested) {
    ...
    const auto* outer_tuple = require_alt<tree::TupleExpr>(outer_arg.value, "TupleExpr");
    ...
    const auto* inner_tuple = require_alt<tree::TupleExpr>(inner_arg.value, "TupleExpr");
...
TEST(lambda_single_param) {
    ...
    if (lam == nullptr)
        return;
    const auto* p =
        require_alt<tree::VarPattern>(get_pattern(r.prog, lam->param).value, "VarPattern");
    if (p != nullptr)
...
TEST(lambda_multiple_params_in_parens) {
    ...
    if (lam == nullptr)
        return;
    const auto* tup_pat =
        require_alt<tree::TuplePattern>(get_pattern(r.prog, lam->param).value, "TuplePattern");
    if (tup_pat != nullptr)
...
TEST(simple_Binding_to_var_pattern) {
    ...
    if (assign == nullptr)
        return;
    const auto* pat =
        require_alt<tree::VarPattern>(get_pattern(r.prog, assign->target).value, "VarPattern");
    if (pat != nullptr)
...
TEST(tuple_destructuring_Binding) {
    ...
    if (assign == nullptr)
        return;
    const auto* pat =
        require_alt<tree::TuplePattern>(get_pattern(r.prog, assign->target).value, "TuplePattern");
    if (pat == nullptr)
        return;
...
TEST(wildcard_and_var_patterns_in_tuple_destructure) {
    ...
    CHECK(!r.diag.has_errors());
    const auto* assign =
        require_alt<tree::Binding>(get_expr(r.prog, r.prog.exprs[0]).value, "Binding");
    if (assign == nullptr)
        return;
    const auto* pat =
        require_alt<tree::TuplePattern>(get_pattern(r.prog, assign->target).value, "TuplePattern");
    ...
    require_alt<tree::WildcardPattern>(get_pattern(r.prog, pat->fields[0].pattern).value,
                                       "WildcardPattern");
    require_alt<tree::WildcardPattern>(get_pattern(r.prog, pat->fields[1].pattern).value,
                                       "WildcardPattern");
    const auto* third =
        require_alt<tree::VarPattern>(get_pattern(r.prog, pat->fields[2].pattern).value,
                                      "VarPattern");
    if (third != nullptr)
...
TEST(named_field_pattern_with_catchall_wildcard) {
    ...
    const auto* px =
        require_alt<tree::VarPattern>(get_pattern(r.prog, pat->fields[0].pattern).value,
                                      "VarPattern");
    if (px != nullptr)
...
TEST(function_clause_with_tuple_param_pattern) {
    ...
    const auto* lambda = require_alt<tree::Lambda>(get_expr(r.prog, assign->value).value, "Lambda");
    if (lambda == nullptr)
        return;
    CHECK(lambda->name.has_value());
    CHECK_EQ(*lambda->name, std::string("add"));
    const auto* tup_pat =
        require_alt<tree::TuplePattern>(get_pattern(r.prog, lambda->param).value, "TuplePattern");
    if (tup_pat != nullptr)
        CHECK_EQ(tup_pat->fields.size(), static_cast<size_t>(2));
...
TEST(function_clause_with_literal_pattern) {
    ...
    const auto* lit_pat =
        require_alt<tree::FloatPattern>(get_pattern(r.prog, lambda->param).value, "FloatPattern");
    if (lit_pat != nullptr)
...
TEST(function_clause_with_nested_tuple_pattern) {
    ...
    CHECK_EQ(tup_pat->fields.size(), static_cast<size_t>(2));
    const auto* first =
        require_alt<tree::VarPattern>(get_pattern(r.prog, tup_pat->fields[0].pattern).value,
                                      "VarPattern");
    (void)first;
    const auto* nested =
        require_alt<tree::TuplePattern>(get_pattern(r.prog, tup_pat->fields[1].pattern).value,
                                        "TuplePattern");
    if (nested == nullptr)
        return;
...
TEST(multiple_top_level_statements) {
    ...
    CHECK_EQ(r.prog.exprs.size(), static_cast<size_t>(2));
    require_alt<tree::Binding>(get_expr(r.prog, r.prog.exprs[0]).value, "Binding");
    require_alt<tree::Binding>(get_expr(r.prog, r.prog.exprs[1]).value, "Binding");
}
...
TEST(deep_nested_named_tuple_destructure) {
    ...
    const auto* nested =
        require_alt<tree::TuplePattern>(get_pattern(r.prog, pat->fields[1].pattern).value,
                                        "TuplePattern");
    if (nested != nullptr)
        CHECK_EQ(nested->fields.size(), static_cast<size_t>(2));
    CHECK(!pat->fields[2].name.has_value());
    require_alt<tree::WildcardPattern>(get_pattern(r.prog, pat->fields[2].pattern).value,
                                       "WildcardPattern");
}
...
TEST(unary_minus_binds_tighter_than_multiplication) {
    ...
    if (mul == nullptr)
        return;
    CHECK(mul->op == BinaryOp::Mul);
    const auto* sub = require_alt<tree::BinaryExpr>(get_expr(r.prog, mul->lhs).value, "BinaryExpr");
    if (sub != nullptr)
        CHECK(sub->op == BinaryOp::Sub);
}
...
TEST(unary_minus_applies_to_entire_postfix_expression) {
    ...
    if (sub == nullptr)
        return;
    CHECK(sub->op == BinaryOp::Sub);
    const auto* call = require_alt<tree::Call>(get_expr(r.prog, sub->rhs).value, "Call");
    CHECK(call != nullptr);
}
...
TEST(chained_unary_minus_nests) {
    ...
    const auto* middle =
        require_alt<tree::BinaryExpr>(get_expr(r.prog, outer->rhs).value, "BinaryExpr");
...
TEST(full_operator_precedence_chain) {
    ...
    if (eq == nullptr)
        return;
    CHECK(eq->op == BinaryOp::Equal);
    const auto* add = require_alt<tree::BinaryExpr>(get_expr(r.prog, eq->lhs).value, "BinaryExpr");
    if (add == nullptr)
        return;
    CHECK(add->op == BinaryOp::Add);
    const auto* mul = require_alt<tree::BinaryExpr>(get_expr(r.prog, add->rhs).value, "BinaryExpr");
    if (mul != nullptr)
        CHECK(mul->op == BinaryOp::Mul);
}
...
TEST(parentheses_override_precedence) {
    ...
    if (mul == nullptr)
        return;
    CHECK(mul->op == BinaryOp::Mul);
    const auto* add = require_alt<tree::BinaryExpr>(get_expr(r.prog, mul->lhs).value, "BinaryExpr");
    if (add != nullptr)
        CHECK(add->op == BinaryOp::Add);
}
...
TEST(call_with_one_unary_argument) {
    ...
    CHECK(!r.diag.has_errors());
    const auto* call = require_alt<tree::Call>(get_expr(r.prog, r.prog.exprs[0]).value, "Call");
    if (call == nullptr)
        return;
    const tree::Expr& arg = get_expr(r.prog, call->arg);
...
TEST(chained_calls_preserve_left_associativity) {
    ...
    CHECK(!r.diag.has_errors());
    const auto* outer = require_alt<tree::Call>(get_expr(r.prog, r.prog.exprs[0]).value, "Call");
    CHECK(outer != nullptr);
    if (outer == nullptr)
        return;
    const auto* middle = require_alt<tree::Call>(get_expr(r.prog, outer->callee).value, "Call");
    CHECK(middle != nullptr);
    if (middle == nullptr)
        return;
    const auto* inner = require_alt<tree::Call>(get_expr(r.prog, middle->callee).value, "Call");
    CHECK(inner != nullptr);
}
...
TEST(lambda_body_includes_full_expression) {
    ...
    CHECK(!r.diag.has_errors());
    const auto* lam = require_alt<tree::Lambda>(get_expr(r.prog, r.prog.exprs[0]).value, "Lambda");
    if (lam == nullptr)
        return;
    const auto* add =
        require_alt<tree::BinaryExpr>(get_expr(r.prog, lam->body).value, "BinaryExpr");
    if (add == nullptr)
        return;
    CHECK(add->op == BinaryOp::Add);
    const auto* mul = require_alt<tree::BinaryExpr>(get_expr(r.prog, add->rhs).value, "BinaryExpr");
...
TEST(nested_lambdas) {
    ...
    CHECK(!r.diag.has_errors());
    const auto* outer =
        require_alt<tree::Lambda>(get_expr(r.prog, r.prog.exprs[0]).value, "Lambda");
    if (outer == nullptr)
        return;
    const auto* inner = require_alt<tree::Lambda>(get_expr(r.prog, outer->body).value, "Lambda");
    CHECK(inner != nullptr);
}
