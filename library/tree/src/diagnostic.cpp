...
Diagnostic& DiagnosticEngine::report(Severity severity, Span span, std::string message) {
    diagnostics_.push_back(
        Diagnostic{.severity = severity, .span = span, .message = std::move(message)});
...
size_t DiagnosticEngine::count(Severity severity) const noexcept {
    return static_cast<size_t>(
        std::count_if(diagnostics_.begin(), diagnostics_.end(), [severity](const Diagnostic& d) {
...
void DiagnosticEngine::print_location(std::ostream& os, Span span) const {
    ...
    const int underline_end =
        (span.end.line == span.begin.line) ? span.end.column : static_cast<int>(text.size()) + 1;
...
void DiagnosticEngine::print_one(std::ostream& os, const Diagnostic& diag) const {
...
void DiagnosticEngine::print_all(std::ostream& os) const {
    ...
    std::ranges::transform(diagnostics_, std::back_inserter(sorted), [](const Diagnostic& d) {
        return &d;
    });
