...
void print_pattern(const tree::Arena& arena,
                   tree::PatternId id,
                   std::string& out,
                   const std::string& prefix,
                   bool is_last);
...
void print_pattern(const tree::Arena& arena,
    ...
    match(
        ...
        [&](const tree::FloatPattern& f) {
            append_line(out, prefix, is_last, "Float(" + format_number(f.value) + ")");
        },
        ...
        [&](const tree::TuplePattern& t) {
            ...
            print_pattern_fields(arena, t.fields, out, child_prefix(prefix, is_last));
        });
}

void print_expr(const tree::Arena& arena,
                tree::ExprId id,
                std::string& out,
                const std::string& prefix,
                bool is_last);
    ...
    match(
        ...
        [&](const tree::MultiClauseLambda& m) {
            ...
            for (size_t i = 0; i < m.clauses.size(); ++i) {
                ...
                append_line(out, cp, clause_last, "clause " + std::to_string(i) + ":");
                ...
                print_pattern(arena,
                              m.clauses[i].param,
                              out,
                              child_prefix(clause_prefix, false),
                              true);
                append_line(out, clause_prefix, true, "body:");
                print_expr(arena, m.clauses[i].body, out, child_prefix(clause_prefix, true), true);
            }
        });
}
...
PatternId Arena::clone(const Arena& src, PatternId id) {
    ...
    return match(
        ...
        [&](const TuplePattern& t) {
            ...
            std::ranges::transform(t.fields, std::back_inserter(fields), [&](const auto& f) {
                return TuplePatternField{.name = f.name, .pattern = clone(src, f.pattern)};
            });
...
ExprId Arena::clone(const Arena& src, ExprId id) {
    ...
    return match(
        ...
        [&](const TupleExpr& t) {
            ...
            std::ranges::transform(t.fields, std::back_inserter(fields), [&](const auto& f) {
                return TupleExprField{.name = f.name, .value = clone(src, f.value)};
            });
        ...
        [&](const BinaryExpr& b) {
            return make_expr<BinaryExpr>(e.span, b.op, clone(src, b.lhs), clone(src, b.rhs));
        },
        [&](const Lambda& l) {
            return make_expr<Lambda>(e.span,
                                     std::optional<std::string>(l.name),
                                     clone(src, l.param),
                                     clone(src, l.body));
        },
        [&](const Binding& a) {
            return make_expr<Binding>(e.span, clone(src, a.target), clone(src, a.value));
        },
        [&](const MultiClauseLambda& m) {
            std::vector<LambdaClause> clauses;
            clauses.reserve(m.clauses.size());

            std::ranges::transform(m.clauses, std::back_inserter(clauses), [&](const auto& c) {
                return LambdaClause{.param = clone(src, c.param), .body = clone(src, c.body)};
            });

            return make_expr<MultiClauseLambda>(e.span,
                                                std::optional<std::string>(m.name),
                                                std::move(clauses));
        });
}
...
Program clone(const Program& prog) {
    ...
    std::ranges::transform(prog.exprs, std::back_inserter(result.exprs), [&](ExprId id) {
        return result.arena.clone(prog.arena, id);
    });
